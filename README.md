...
秋招期间，我需要同时处理大量不同类型的事情：跟踪不同公司的投递进度、完成限时测评、准备和参加多轮面试，同时还要继续推进自己的科研工作。这些任务来自不同的平台，有着不同的截止时间和优先级。投递数量增加以后，仅靠记忆或零散的备忘录，很容易漏掉测评截止时间、面试准备事项，或者因为临时安排打乱原本的科研计划。

我尝试过使用传统的待办软件，但我需要的不只是记录任务。我希望能快速知道今天必须完成什么、哪些事情已经积压、接下来几天有哪些测评或面试，还希望在计划发生变化时，可以直接用自然语言调整安排。更重要的是，我希望数据保存在自己的电脑上，不依赖某个平台的账号和云端服务。

因此，我做了 Today List。它既是一个可以直接使用的本地每日清单，也是一个 MCP Server。浏览器、Windows 桌面小窗和 AI Agent 使用同一份任务数据；我可以手动管理任务，也可以让 Agent 帮我添加投递安排、调整面试准备的优先级、查看即将到期的事项，并复盘最近一段时间的完成情况。

```text
记录投递与科研任务 → 安排测评和面试 → 跟踪每日进度 → 处理积压 → 定期复盘
```

我希望它解决的不是“再做一个待办软件”，而是在高压力、多线程的秋招阶段，为自己建立一个简单、可靠并且可以被 AI 理解的行动系统。所有任务都保存在本地文件中，人和 Agent 看到的是同一份真实状态，让 Agent 不只是给出泛泛的建议，而是能够基于我的实际安排协助执行和调整计划。

这个项目也记录了我对 MCP 的一次实践：当任务数据能够通过标准工具暴露给 Agent 后，自然语言就可以真正连接到日常工作流，而不再停留在聊天窗口里。
